...
const CITY_META = [
  ...
  // CareBuddy hub → col:1 row:1 (center)
 ...
 * CareBuddy hub centre = (150, 150)
 */
function cellCentre(col: number, row: number) {
  return { x: col * 100 + 50, y: row * 100 + 50 }
}

const CareBuddy = { x: 150, y: 150 } // centre cell

type City = (typeof CITY_META)[number] & { feature: string }

/** Animated dot that travels from CareBuddy → city */
function Pulse({ city, color, delay }: { city: City; color: string; delay: number }) {
  ...
  return (
    <motion.circle
      r={4}
      fill={color}
      initial={{ cx: CareBuddy.x, cy: CareBuddy.y, opacity: 0, scale: 0 }}
      animate={{
        cx: [CareBuddy.x, to.x],
        cy: [CareBuddy.y, to.y],
        opacity: [0, 1, 1, 0],
...
export function CityGrid() {
  ...
  /* build a 3×3 slot array (null = CareBuddy hub) */
  const slots: (City | null)[] = Array(9).fill(null)
  CITIES.forEach(c => { slots[c.row * 3 + c.col] = c })
  // slot index 4 (col:1 row:1) stays null → CareBuddy hub
        ...
        {CITIES.map((city, i) => {
          ...
          return (
            <g key={city.name}>
              {/* Static dashed line */}
              <line
                x1={CareBuddy.x} y1={CareBuddy.y} x2={to.x} y2={to.y}
      ...
      <div className="relative grid grid-cols-3 gap-3 auto-rows-[110px]">
        {slots.map((city) => {
          if (city === null) {
            /* ── CareBuddy hub ── */
            return (
              <div key="carebuddy-hub" className="flex items-center justify-center">

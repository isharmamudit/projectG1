...
/**
 * Replaces the static "English" button: opens a panel of all 12 supported
 * languages (native script + English name). Picking one updates the
 * LanguageContext, which re-renders every string wired to useLanguage().
 */
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="glass absolute bottom-full left-0 z-30 mb-2 w-full min-w-[220px] rounded-2xl p-2 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.45)]"
          >
            <p className="px-2.5 pt-1 pb-2 text-[10px] font-black tracking-widest text-fg-muted uppercase">{t.chooseLanguage}</p>
            <ul className="grid max-h-72 grid-cols-1 gap-1 overflow-y-auto">
              {LANGUAGES.map((lang) => (
                <li key={lang.code}>
                  <button
                    type="button"

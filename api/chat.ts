...
// Kept fully self-contained — no imports from src/lib/languages.ts (Vercel's
// function bundler doesn't share tsconfig.app.json's "@/*" paths) and, as of
// this file, no relative imports of api/ siblings either: a same-directory
// relative import (api/_icmrGuidelines.ts) worked fine under `vercel dev`
// but crashed in real production with ERR_MODULE_NOT_FOUND — the sibling
// file wasn't actually included in that function's deployed bundle. Every
// serverless function here must be a fully standalone file.
...
// Hand-distilled summaries (not verbatim reproductions) of real, official
// ICMR Standard Treatment Workflow PDFs, cited by source. Simple keyword
// match, not vector search — fine for a handful of documents.
...
function systemInstructionFor(languageCode: string, retrievedGuidance: ReturnType<typeof retrieveIcmrGuidance>) {
  ...
  return [
    ...
    ``,
    `You are CareBuddy (internally: DoctorLLM India), a specialized medical consultation, clinical-intake, triage and patient-education assistant for Indian patients. Always reply in ${languageName}, regardless of what language the user's message is written in, unless they clearly ask to switch languages.`,
    ``,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!upstream.ok) {
      ...
      console.warn('Primary Groq model failed, retrying with fallback model:', upstream.status, errBody)
    ...
    if (!upstream.ok) {
      const errBody = await upstream.text()
      console.error('Groq request failed on fallback:', upstream.status, errBody)
      res.status(502).json({ error: 'Failed to get a response', details: errBody })
      return
    }

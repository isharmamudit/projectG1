...
// Fully self-contained — a same-directory relative import (api/_clinicalScores.ts)
...
//
// Deterministic score calculators: the rule is that an LLM must never
// "mentally approximate" a validated clinical score — it should gather the
// exact required inputs and call a real calculator instead. These are pure
// functions with no LLM involvement, so a score is either computed
// correctly from real inputs or not computed at all.
...
function calculateTimiScore(inputs: TimiInputs) {
...
function calculateGraceScore(i: GraceInputs) {
...
function calculateCurb65(inputs: Curb65Inputs) {
  const score = Object.values(inputs).filter(Boolean).length
...
function calculateQsofa(inputs: QsofaInputs) {
  const score = Object.values(inputs).filter(Boolean).length

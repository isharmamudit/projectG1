...
// Kept fully self-contained (see api/chat.ts) — no imports from src/lib, and
// no relative imports of api/ siblings either: that pattern (api/_icmrGuidelines.ts)
// worked under `vercel dev` but crashed in real production with
// ERR_MODULE_NOT_FOUND, since the sibling file wasn't actually included in
// this function's deployed bundle. Every function here must be standalone.
//
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!upstream.ok) {
      const errBody = await upstream.text()
      console.error('Groq voice-chat request failed on fallback:', upstream.status, errBody)
